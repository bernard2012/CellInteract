...
if __name__=="__main__":
	parser = argparse.ArgumentParser(description="Binning Gene Expression", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
	parser.add_argument("--do-organism", dest="organism", type=str, choices=["human", "mouse"], required=True, help="which organism to bin: human or mouse")
	...
	if args.organism=="mouse":
		...
		print(len(to_do_mouse))
		member = np.empty((len(to_do_mouse), mat.shape[1]), dtype="int32")
		...
		for ig,gg in enumerate(to_do_mouse):
			...
			member[ig,:] = kmeans.labels_
			print(ig, gg, kmeans.cluster_centers_)

		fw = open("%s/mouse_genes_cell_interactions_clusters.pkl" % args.inputdir, "wb")
		pickle.dump([to_do_mouse, member], fw)
		fw.close()
	...
	if args.organism=="human":
		...
		member = np.empty((len(to_do_human), mat2.shape[1]), dtype="int32")

		#print(len(to_do_human))
		...
		fw = open("%s/human_genes_cell_interactions_clusters.pkl" % args.inputdir, "wb")
		pickle.dump([to_do_human, member], fw)
		fw.close()
