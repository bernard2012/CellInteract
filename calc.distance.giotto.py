...
if __name__=="__main__":
	...
	parser.add_argument("--do-organism", dest="organism", type=str, choices=["human", "mouse"], required=True, help="which organism to bin: human, mouse")
	...
	if args.organism=="mouse":
		f = open("%s/mouse_genes_cell_interactions_clusters.pkl" % args.inputdir, "rb")
		[to_do_mouse, member] = pickle.load(f)
		f.close()
		map_todo_mouse = {}
		for ig,g in enumerate(to_do_mouse):
			map_todo_mouse[g] = ig
	
		tot_spot = member.shape[1]
		to_do_mouse = []
		pairs = []
		for g1,g2 in db:
			...
			for n_g1 in m1s:
				...
				for n_g2 in m2s:
					if n_g2 not in map_gene:
						continue
					pairs.append((n_g1, n_g2))

		pairs = set(pairs)
		outlines = []
		cur_ind = 0
		tot_ind = len(pairs)
		for n_g1, n_g2 in pairs:
			m_g1 = member[map_todo_mouse[n_g1],:]
			m_g2 = member[map_todo_mouse[n_g2],:]
			...
			print("%d/%d" % (cur_ind+1, tot_ind), n_g1, n_g2, np.mean(vals))
			outlines.append((n_g1, n_g2, np.mean(vals)))

		outlines = sorted(outlines, key=itemgetter(2), reverse=True)
		fw =open("%s/mouse_stroma_stroma_interactions.txt" % args.inputdir, "w")
	...
	if args.organism=="human":
		f = open("%s/human_genes_cell_interactions_clusters.pkl" % args.inputdir, "rb")
		[to_do_human, member] = pickle.load(f)
		f.close()
		map_todo_human = {}
		...
		tot_ind = len(pairs)
		for n_g1,n_g2 in pairs:
			m_g1 = member[map_todo_human[n_g1],:]
			m_g2 = member[map_todo_human[n_g2],:]
			...
			for ind1 in indices1:
				for ind2 in indices2:
					...
					for xi in range(1000):
						rand1 = random.randint(tot_spot, size=(ind1.shape[0]))
						rand2 = random.randint(tot_spot, size=(ind2.shape[0]))
						rand_ov.append(np.sum(adj[np.ix_(rand1, rand2)]))
					ov = np.sum(adj[np.ix_(ind1, ind2)])
					t_mean = np.mean(rand_ov)
					t_std = np.std(rand_ov)
					vals.append((ov - t_mean)/t_std)
		...
		outlines = sorted(outlines, key=itemgetter(2), reverse=True)
		fw =open("%s/human_epithelium_epithelium_interactions.txt" % args.inputdir, "w")
		for i,j,k in outlines:
			fw.write("%s\t%s\t%f\n" % (i,j,k))
		fw.close()
